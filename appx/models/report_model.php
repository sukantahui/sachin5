...
class report_model extends CI_Model {
    ...
    function select_report_details_by_date_to_date($start_date,$end_date){
        $sql="select 
            DATE_FORMAT(transaction_date, \"%d/%m/%Y\") as transaction_date
            ,sum(inward) as mseed_inward
            , sum(outward) as mseed_outward
            , get_opening_stock(1,transaction_date) + sum(inward) - sum(outward) as mseed_closing_balance
            ,get_moil_production(transaction_date) as produced_moil
            ,get_oil_cake_production(transaction_date) as produced_cake
            
            ,sum(outward)-(get_moil_production(transaction_date) + get_oil_cake_production(transaction_date)) as wastage
            from stock_transaction
            where date(transaction_date) between ? and ? and product_id=1 group by transaction_date";
